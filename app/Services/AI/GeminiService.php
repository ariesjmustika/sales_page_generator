...
class GeminiService implements AIServiceInterface
{
    ...
    protected array $models = [
        'gemini-2.5-flash',         // Primary — highest success rate
    ...
    protected function isRateLimited(string $key, string $model): bool
    {
        return Cache::has($this->rateLimitCacheKey($key, $model));
    }

    protected function markRateLimited(string $key, string $model): void
    {
        Cache::put($this->rateLimitCacheKey($key, $model), true, now()->addSeconds(60));
    }

    protected function rateLimitCacheKey(string $key, string $model): string
    {
        return 'gemini_429_' . $model . '_' . substr($key, 0, 8);
    }
    ...
    {
        $keys = array_filter(array_map('trim', explode(',', $this->apiKey)), fn($k) => !empty($k) && $k !== 'MOCK_KEY');

        if (empty($keys)) return null;

        // Loop model dulu, baru key — supaya spreading quota lebih merata
        foreach ($this->models as $model) {
            foreach ($keys as $currentKey) {

                if ($this->isRateLimited($currentKey, $model)) {
                    Log::info("Skipping rate-limited key/model: {$model} / " . substr($currentKey, 0, 8));
                    continue;
                }
                ...
                try {
                    ...
                    Log::info("Trying Gemini — model: {$model} | key: " . substr($currentKey, 0, 8));
                    ...
                    if ($response->successful()) {
                        Log::info("Gemini success — model: {$model} | key: " . substr($currentKey, 0, 8));
                        return $response->json()['candidates'][0]['content']['parts'][0]['text'] ?? null;
                    }

                    if ($response->status() === 429) {
                        Log::warning("429 Rate Limited — model: {$model} | key: " . substr($currentKey, 0, 8) . " (cooldown 60s)");
                        $this->markRateLimited($currentKey, $model);
                        continue;
                    }

                    Log::warning("Gemini failed — model: {$model}", [
                        'status' => $response->status(),
                        'body'   => $response->body(),
                    ]);

                } catch (\Exception $e) {
                    Log::error("Gemini exception — model: {$model} | " . $e->getMessage());
                }
            }
        }

        Log::error("All Gemini keys and models exhausted. Falling back to mock.");
        return null;
    }
    ...
    public function generateSalesPage(SalesPageData $data): array
    {
        ...
        $response = $this->callGemini($this->buildPrompt($data), true);
    ...
    public function regenerateSection(SalesPageData $data, string $section, array $currentCopy): array
    {
        ...
        $prompt = $this->buildPrompt($data) . "\n\nCRITICAL: The user wants to REGENERATE only the '{$section}' section. "
            . "Please provide a new, improved version of '{$section}' while keeping the context of the rest of the page. "
            . "Return the FULL JSON with the updated '{$section}'.";
    ...
    public function magicRewrite(string $text, string $action): string
    {
        ...
        $prompt = <<<PROMPT
        You are a master conversion copywriter. Rewrite the sales copy below to be {$action}.
        ...
        return $response ? trim($response, " \"\n\r") : $this->getMockRewrite($text, $action);
    }
    ...
    protected function getMockResponse(SalesPageData $data): array
    {
        return [
            ...
            'features_breakdown'       => array_map(
                fn($f) => ['title' => $f, 'description' => "Detailed breakdown of how {$f} helps you achieve more."],
